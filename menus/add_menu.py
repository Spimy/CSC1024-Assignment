...
class AddMenu(BaseMenu):
    ...
    def selection(self):
        ...
        while True:
            ...
            while True:
                publisher = input(
                    f"{'[Publisher should not consist a comma(s) or be empty] ' if self.error_flags['publisher'] else ''}Enter the Publisher: "
                )

                # Guard Clause - if publisher is valid then break
                # If not valid - set error_flag to true
                if self.root.validator.is_valid_string(publisher):
                    break
            ...
            while True:
                genre = input(
                    f"{'[Genre should not consist a comma(s) or be empty] ' if self.error_flags['genre'] else ''}Enter the Genre: "
                )

                # Guard Clause - if genre is valid then break
                # If not valid - set error_flag to true
                if self.root.validator.is_valid_string(genre):
                    break
